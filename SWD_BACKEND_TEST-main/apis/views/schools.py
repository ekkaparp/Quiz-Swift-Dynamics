    ...
    def post(request, *args, **kwargs):
        ...
        rules:      - Score must be number, equal or greater than 0 and equal or less than 100.
                    ...
                    - Payload data must be contained `first_name`, `last_name`, `subject_title` and `score`.
                        ...
                        (ถ้าชนิดข้อมูลไม่ถูกต้องให้ return bad request status)
                    ...
                    (ถ้าไม่พบข้อมูล return bad request status)
        ...
        if student_first_name == None or not student_first_name.isalpha() :
            return Response(status=status.HTTP_400_BAD_REQUEST)

        elif student_last_name == None or not student_last_name.isalpha() :
            return Response(status=status.HTTP_400_BAD_REQUEST)

        elif subjects_title == None or not subjects_title.isalpha() :
            return Response(status=status.HTTP_400_BAD_REQUEST)

        elif str(new_score) == None or not str(new_score).isalpha() :
            return Response(status=status.HTTP_400_BAD_REQUEST)
        
        try :
            subjects = Subjects.objects.get(title = subjects_title)
        except :
            return Response(status=status.HTTP_400_BAD_REQUEST)
